...
import { classifyHazard, ClassificationResult } from '../../modules/processing/hazard-classification';
...
const VALID_SEVERITY_LEVELS: SeverityLevel[] = ['low', 'moderate', 'high'];
...
const hazardKeywordTextArb = fc.oneof(
  fc.constant('There is severe flooding in the coastal area'),
  fc.constant('Storm surge warning issued for the beach'),
  fc.constant('High waves detected near the shore'),
  fc.constant('Erosion observed along the cliff'),
  fc.constant('Dangerous rip current reported by swimmers'),
  fc.constant('Tsunami alert for the coastal region'),
  fc.constant('Oil pollution spotted in the water'),
  fc.constant('Minor water level rise observed'),
  fc.constant('Moderate storm conditions expected'),
  fc.constant('Critical emergency situation developing'),
  // Random text that may or may not contain keywords
  reportTextArb
);

// Arbitrary for generating optional media URLs
const mediaUrlsArb = fc.option(
 ...
 */
function isValidClassificationResult(result: ClassificationResult): boolean {
  // Check hazardType is valid
  if (!isValidHazardType(result.hazardType)) {
    return false;
  }

  // Check severity is valid
  if (!isValidSeverityLevel(result.severity)) {
    return false;
  }
...
describe('Property 7: Classification Output Validity', () => {
  ...
  it('should always return a valid hazardType from the defined enum', async () => {
    await fc.assert(
      fc.asyncProperty(hazardKeywordTextArb, async (text) => {
        // Force keyword-based classification to avoid external API calls
        const result = await classifyHazard(text, undefined, { forceKeywords: true });
        
        // Verify hazardType is one of the valid types
        expect(VALID_HAZARD_TYPES).toContain(result.hazardType);
        expect(isValidHazardType(result.hazardType)).toBe(true);
  ...
  it('should always return a valid severity level (low, moderate, high)', async () => {
    await fc.assert(
      fc.asyncProperty(hazardKeywordTextArb, async (text) => {
        ...
        expect(isValidSeverityLevel(result.severity)).toBe(true);
  ...
  it('should return a complete valid classification result for any input text', async () => {
    await fc.assert(
      fc.asyncProperty(hazardKeywordTextArb, mediaUrlsArb, async (text, mediaUrls) => {
        // Force keyword-based classification to avoid external API calls
        const result = await classifyHazard(text, mediaUrls, { forceKeywords: true });
  ...
  it('should correctly classify text containing flooding keywords', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(
          'flood water rising in the streets',
          'severe flooding reported downtown',
          'water submerged the parking lot',
          'inundation of coastal properties'
        ),
        async (floodText) => {
          const result = await classifyHazard(floodText, undefined, { forceKeywords: true });
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(
          ...
          'coastal storm surge expected',
          'storm tide approaching the shore'
        ),
        async (surgeText) => {
          const result = await classifyHazard(surgeText, undefined, { forceKeywords: true });
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(
          ...
          'dangerous critical situation',
          'life-threatening storm surge'
        ),
        async (severeText) => {
          const result = await classifyHazard(severeText, undefined, { forceKeywords: true });
  ...
  it('should assign low severity for text with minor/minimal indicators', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(
          'minor flooding observed',
          'small wave activity',
          'slight erosion noticed',
          'minimal water level change'
        ),
        async (minorText) => {
          const result = await classifyHazard(minorText, undefined, { forceKeywords: true });
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom(
          ...
          'report from coastal area',
          'situation developing nearby',
          'general observation noted',
          'incident reported today'
        ),
        async (genericText) => {
          const result = await classifyHazard(genericText, undefined, { forceKeywords: true });
  ...
  it('should handle empty or whitespace-only text gracefully', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom('', '   ', '\t\n', '  \n  '),
        async (emptyText) => {
          const result = await classifyHazard(emptyText, undefined, { forceKeywords: true });
  ...
  it('should increase confidence with more keyword matches', async () => {
    ...
    const singleKeywordResult = await classifyHazard('flood', undefined, { forceKeywords: true });
    const multipleKeywordResult = await classifyHazard('flood water rising overflow', undefined, { forceKeywords: true });
