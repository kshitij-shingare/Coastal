...
import { validateReport, Report, HazardType, SeverityLevel, SourceType, ReportStatus } from '../../../shared/types/report';
...
const validReportArb: fc.Arbitrary<Partial<Report>> = fc.record({
  id: fc.uuid(),
  timestamp: fc.date(),
  ...
  it('should reject reports with missing location', () => {
    fc.assert(
      fc.property(
        ...
        sourceTypeArb,
        (content, source) => {
          const reportWithoutLocation: Partial<Report> = {
            content,
            source,
            // location is missing
          };

          const errors = validateReport(reportWithoutLocation);
    ...
    fc.assert(
      fc.property(
        ...
        (location, source) => {
          ...
          const reportWithoutDescription: Partial<Report> = {
            location,
            source,
            content: {
          ...
          const errors = validateReport(reportWithoutDescription);
  ...
  it('should reject reports with invalid source type', () => {
    fc.assert(
      fc.property(
        validLocationArb,
        validContentArb,
        fc.string({ minLength: 1, maxLength: 20 }).filter(s => !['citizen', 'social', 'official'].includes(s)),
        (location, content, invalidSource) => {
          const reportWithInvalidSource: Partial<Report> = {
            location,
            content,
            source: invalidSource as SourceType, // Invalid source
          };

          const errors = validateReport(reportWithInvalidSource);
  ...
  it('should reject reports with invalid latitude (out of range)', () => {
    fc.assert(
      fc.property(
        ...
        validContentArb,
        sourceTypeArb,
        (invalidLat, validLon, content, source) => {
          const reportWithInvalidLat: Partial<Report> = {
            location: {
              latitude: invalidLat,
              longitude: validLon,
            },
          ...
          const errors = validateReport(reportWithInvalidLat);
  ...
  it('should reject reports with invalid longitude (out of range)', () => {
    fc.assert(
      fc.property(
        ...
        validContentArb,
        sourceTypeArb,
        (validLat, invalidLon, content, source) => {
          const reportWithInvalidLon: Partial<Report> = {
            location: {
              latitude: validLat,
          ...
          const errors = validateReport(reportWithInvalidLon);
  ...
  it('should reject reports with invalid confidence score (out of 0-100 range)', () => {
    fc.assert(
      fc.property(
        ...
        (location, content, source, invalidConfidence) => {
          const reportWithInvalidConfidence: Partial<Report> = {
            location,
            content,
            source,
            classification: {
          ...
          const errors = validateReport(reportWithInvalidConfidence);
  ...
  it('should list all missing fields when multiple required fields are missing', () => {
    fc.assert(
      fc.property(
        fc.boolean(),
        fc.boolean(),
        (missingLocation, missingContent) => {
          ...
          const errors = validateReport(partialReport);
  ...
  it('should handle whitespace-only description as invalid', () => {
    fc.assert(
      fc.property(
        ...
        (location, source, whitespaceText) => {
          const reportWithWhitespaceDescription: Partial<Report> = {
            location,
            source,
            content: {
          ...
          const errors = validateReport(reportWithWhitespaceDescription);
