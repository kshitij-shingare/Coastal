 ...
 */
function alertsRepresentSameIncident(alert1: Alert, alert2: Alert): boolean {
  // Must be same hazard type
  if (alert1.hazardType !== alert2.hazardType) return false;
  
  // Must be same region
  if (alert1.region.name !== alert2.region.name) return false;
...
const reportArb = (
  ...
  return fc.record({
    ...
    status: fc.constant('pending' as ReportStatus),
...
const alertArb = (
  ...
  return fc.record({
    ...
    status: fc.constant('active' as AlertStatus),
 ...
 */
function shouldCreateNewAlert(
  existingAlerts: Alert[],
  newAlertCandidate: { hazardType: HazardType; region: string; timestamp: Date }
): boolean {
  for (const existing of existingAlerts) {
    ...
    if (existing.hazardType !== newAlertCandidate.hazardType) continue;
    
    // Check if same region
    if (existing.region.name !== newAlertCandidate.region) continue;
 ...
 */
function processReportClustersWithDeduplication(
  clusters: Array<{ hazardType: HazardType; region: string; timestamp: Date; reportIds: string[] }>
): Alert[] {
  const alerts: Alert[] = [];
...
describe('Property 11: Alert Deduplication', () => {
  ...
  it('should have at most one active alert per incident (same hazard type, region, and time window)', () => {
    fc.assert(
      fc.property(
        ...
        fc.constantFrom('North Coast', 'South Beach', 'East Harbor', 'West Bay'),
        // Generate multiple clusters for the same incident
        fc.integer({ min: 2, max: 5 }),
        (hazardType, region, clusterCount) => {
          ...
          const activeAlertsForIncident = alerts.filter(a =>
            a.status === 'active' &&
  ...
  it('should allow separate alerts for different hazard types in the same region', () => {
    fc.assert(
      fc.property(
        fc.constantFrom('North Coast', 'South Beach', 'East Harbor', 'West Bay'),
        fc.array(fc.constantFrom(...HAZARD_TYPES), { minLength: 2, maxLength: 4 }),
        (region, hazardTypes) => {
  ...
  it('should correctly identify same incident based on all criteria', () => {
    fc.assert(
      fc.property(
        ...
        fc.constantFrom('North Coast', 'South Beach', 'East Harbor', 'West Bay'),
        fc.constantFrom('North Coast', 'South Beach', 'East Harbor', 'West Bay'),
        fc.integer({ min: 0, max: 100 }), // hours apart
        (hazardType1, hazardType2, region1, region2, hoursDiff) => {
          ...
          const expectedSameIncident = 
            hazardType1 === hazardType2 &&
            region1 === region2 &&
  ...
  it('should not deduplicate resolved or false_alarm alerts', () => {
    fc.assert(
      fc.property(
        fc.constantFrom(...HAZARD_TYPES),
        fc.constantFrom('North Coast', 'South Beach', 'East Harbor', 'West Bay'),
        fc.constantFrom('resolved', 'false_alarm') as fc.Arbitrary<AlertStatus>,
        (hazardType, region, inactiveStatus) => {
