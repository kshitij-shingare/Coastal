 ...
 */
function calculateStatsDirectly(reports: Report[], alerts: Alert[]): SystemStats {
 ...
 */
function simulateGetSystemStats(reports: Report[], alerts: Alert[]): SystemStats {
    ...
    fc.assert(
      fc.property(
        ...
        (reports, alerts) => {
          ...
          const expectedByRegion: Record<string, number> = {};
          reports.forEach(r => {
            if (r.region) {
    ...
    fc.assert(
      fc.property(
        ...
        (reports) => {
          ...
          const typesInReports = new Set(
            reports.map(r => r.classification.hazardType).filter(Boolean)
          );
