...
import { Report, SourceType, SeverityLevel } from '../../../shared/types/report';
 ...
 */
function calculateSourceCountScore(reportCount: number): number {
  // Logarithmic scaling: more reports = higher confidence, but diminishing returns
  if (reportCount <= 0) return 0;
  if (reportCount === 1) return 30;
  if (reportCount === 2) return 50;
  if (reportCount <= 5) return 60 + (reportCount - 2) * 5;
  if (reportCount <= 10) return 75 + (reportCount - 5) * 2;
 ...
 */
function calculateSourceDiversityScore(reports: Report[]): number {
 ...
 */
function calculateTemporalConsistencyScore(reports: Report[]): number {
  ...
  if (avgInterval < oneHour) return 95;
  if (avgInterval < sixHours) return 80;
  if (avgInterval < oneDay) return 60;
  return 40;
}
 ...
 */
function calculateSpatialConsistencyScore(reports: Report[]): number {
  if (reports.length < 2) return 50;
  ...
  const distances = reports.map(r => 
    haversineDistance(r.location.latitude, r.location.longitude, centroidLat, centroidLng)
  );
  const avgDistance = distances.reduce((a, b) => a + b, 0) / distances.length;

  // Score based on clustering
  if (avgDistance < 1) return 95;  // Within 1km
  if (avgDistance < 5) return 85;  // Within 5km
  if (avgDistance < 10) return 70; // Within 10km
  if (avgDistance < 25) return 55; // Within 25km
  return 40;
}
 ...
 */
function calculateMediaEvidenceScore(reports: Report[]): number {
  const reportsWithMedia = reports.filter(r => 
    r.content.mediaFiles && r.content.mediaFiles.length > 0
  );
  
  if (reportsWithMedia.length === 0) return 30;
  
  const mediaRatio = reportsWithMedia.length / reports.length;
  const totalMediaFiles = reports.reduce((sum, r) => 
    sum + (r.content.mediaFiles?.length || 0), 0
  );
 ...
 */
function calculateAIConfidenceScore(reports: Report[]): number {
 ...
 */
export function calculateConfidenceScore(reports: Report[]): ConfidenceScore {
  if (reports.length === 0) {
    return {
      overall: 0,
      factors: {
  ...
  const overall = Math.round(
    factors.sourceCount * FACTOR_WEIGHTS.sourceCount +
 ...
 */
export function determineSeverity(reports: Report[], confidence: number): SeverityLevel {
