...
export async function connectRedis(): Promise<RedisClientType | null> {
  ...
  if (process.env.DEMO_MODE === 'true') {
    logger.info('Redis connection skipped (DEMO MODE)');
    return null;
  }
  ...
  try {
    ...
    connectionRetries++;
    logger.error(`Redis connection failed (attempt ${connectionRetries}/${MAX_RETRIES}):`, error);

    if (connectionRetries < MAX_RETRIES) {
      logger.info(`Retrying Redis connection in ${RETRY_DELAY / 1000} seconds...`);
