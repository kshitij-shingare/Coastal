...
exports.hazardClassificationService = void 0;
exports.classifyHazard = classifyHazard;
exports.classifyReportsBatch = classifyReportsBatch;
 ...
 */
async function classifyWithOpenAI(text, mediaUrls) {
    if (!OPENAI_API_KEY) {
...
${mediaUrls?.length ? `\nMedia files attached: ${mediaUrls.length}` : ''}`;
    try {
        ...
        logger_1.logger.error('OpenAI classification failed:', error);
        throw error;
    }
}
function validateHazardType(type) {
    const validTypes = ['flooding', 'storm_surge', 'high_waves', 'erosion', 'rip_current', 'tsunami', 'pollution', 'other'];
    return validTypes.includes(type) ? type : 'other';
}
function validateSeverity(severity) {
    const validLevels = ['low', 'moderate', 'high'];
    return validLevels.includes(severity) ? severity : 'moderate';
}
/**
 * Main classification function - uses OpenAI with keyword fallback
 */
async function classifyHazard(text, mediaUrls, options) {
    // Use keyword-based classification in demo mode or when forced
    if (DEMO_MODE || options?.forceKeywords || !OPENAI_API_KEY) {
        logger_1.logger.debug('Using keyword-based classification');
        return classifyWithKeywords(text);
    }
    try {
        logger_1.logger.debug('Using OpenAI classification');
        return await classifyWithOpenAI(text, mediaUrls);
    }
    catch (error) {
        logger_1.logger.warn('OpenAI classification failed, falling back to keywords:', error);
        return classifyWithKeywords(text);
    }
}
/**
 ...
 */
async function classifyReportsBatch(reports) {
    ...
    for (const chunk of chunks) {
        const chunkResults = await Promise.all(chunk.map(async (report) => {
            try {
                const result = await classifyHazard(report.text, report.mediaUrls);
                return { id: report.id, result };
            }
            catch (error) {
                logger_1.logger.error(`Classification failed for report ${report.id}:`, error);
                return {
                    id: report.id,
                    result: {
                        hazardType: 'other',
...
async function generateReportSummary(report) {
    ...
    try {
        ...
        logger_1.logger.error('Summary generation failed:', error);
        return `${report.classification.severity || 'Moderate'} ${report.classification.hazardType || 'hazard'} reported in ${report.region}.`;
    }
}
exports.hazardClassificationService = {
    classifyHazard,
    classifyReportsBatch,
    generateReportSummary,
};
exports.default = exports.hazardClassificationService;
//# sourceMappingURL=hazard-classification.js.map