 ...
 */
function timeFilterToMs(filter) {
 ...
 */
function isWithinTimeWindow(timestamp, filter, referenceTime) {
 ...
 */
const reportWithVariedTimestampArb = (referenceTime) => {
 ...
 */
const alertWithVariedTimestampArb = (referenceTime) => {
    const sixtyDaysAgo = new Date(referenceTime.getTime() - 60 * 24 * 60 * 60 * 1000);
    return fc.record({
        id: fc.uuid(),
 ...
 */
function filterReportsByTimeWindow(reports, filter, referenceTime) {
    return reports.filter(report => isWithinTimeWindow(report.timestamp, filter, referenceTime));
}
/**
 * Filter alerts by time window
 */
function filterAlertsByTimeWindow(alerts, filter, referenceTime) {
...
describe('Property 6: Time-Based Filtering', () => {
    ...
    it('should return only reports within 24h time window', () => {
        fc.assert(fc.property(fc.array(reportWithVariedTimestampArb(referenceTime), { minLength: 1, maxLength: 50 }), (reports) => {
            const filter = '24h';
            ...
            const reportsOutsideWindow = reports.filter(r => !isWithinTimeWindow(r.timestamp, filter, referenceTime));
    ...
    it('should return only alerts within 24h time window', () => {
        fc.assert(fc.property(fc.array(alertWithVariedTimestampArb(referenceTime), { minLength: 1, maxLength: 50 }), (alerts) => {
            const filter = '24h';
    ...
    it('should correctly calculate time window boundaries', () => {
        fc.assert(fc.property(fc.constantFrom(...TIME_FILTERS), fc.date({ min: new Date('2024-01-01'), max: new Date('2025-12-31') }), (filter, testRefTime) => {
            const windowMs = timeFilterToMs(filter);
    ...
    it('should return empty array when no items are within the time window', () => {
        ...
        fc.assert(fc.property(fc.array(fc.record({
            ...
            createdAt: fc.date(),
            updatedAt: fc.date(),
        }), { minLength: 1, maxLength: 20 }), (oldReports) => {
    ...
    it('should preserve all report properties after filtering', () => {
        fc.assert(fc.property(fc.array(reportWithVariedTimestampArb(referenceTime), { minLength: 1, maxLength: 30 }), fc.constantFrom(...TIME_FILTERS), (reports, filter) => {
            const filteredReports = filterReportsByTimeWindow(reports, filter, referenceTime);
    ...
    it('should preserve all alert properties after filtering', () => {
        fc.assert(fc.property(fc.array(alertWithVariedTimestampArb(referenceTime), { minLength: 1, maxLength: 30 }), fc.constantFrom(...TIME_FILTERS), (alerts, filter) => {
            const filteredAlerts = filterAlertsByTimeWindow(alerts, filter, referenceTime);
...
//# sourceMappingURL=time-filtering.test.js.map